...
export function SignalsTable() {
  ...
  async function load() {
    ...
    try {
      ...
      const openSymbols = [...new Set(data.filter((s) => s.status === "OPEN").map((s) => s.symbol))];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <StatCard label="Target Hit" value={stats.target} accent="bull" icon={<TrendingUp className="h-4 w-4" />} />
        <StatCard label="Stop Loss" value={stats.sl} accent="bear" icon={<TrendingDown className="h-4 w-4" />} />
        <StatCard label="Expired" value={stats.expired} accent="muted" icon={<Clock className="h-4 w-4" />} />
      </div>
      ...
      <div className="flex items-center justify-between rounded-lg border bg-card px-4 py-2.5">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          Auto-refreshing every 15s · next in <span className="font-mono text-foreground">{countdown}s</span>
        </div>
      ...
      {error && (
        <div className="rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-sm text-destructive">
          ...
          <div className="text-xs mt-1 opacity-80">
            Check that the backend is running and <code className="font-mono">VITE_API_BASE_URL</code> is set
            correctly.
          </div>
        </div>
      )}
              ...
              {!loading && signals.length === 0 && (
                <tr>
                  <td colSpan={10} className="px-4 py-16 text-center">
                    ...
                    <Button onClick={() => navigate({ to: "/signals/new" })}>Create your first signal</Button>
                  </td>
                </tr>
              )}
              {!loading &&
                signals.map((s) => {
                  const livePrice = s.status === "OPEN" ? livePrices[s.symbol] ?? null : s.realized_price;
                  const roi =
                    s.status === "OPEN"
                      ? computeLiveRoi(s, livePrice)
                      : s.realized_roi;
                  const remaining = s.status === "OPEN" ? timeRemaining(s.expiry_time, now) : "—";
                  return (
                    <tr key={s.id} className="border-b last:border-0 hover:bg-surface-elevated/40 transition-colors">
                      <td className="px-4 py-3 font-mono font-semibold">{s.symbol}</td>
                      <td className="px-4 py-3">
                        <DirectionBadge direction={s.direction} />
                      </td>
                      <td className="px-4 py-3 text-right font-mono">{formatPrice(s.entry_price)}</td>
                      <td className="px-4 py-3 text-right font-mono text-bull">{formatPrice(s.target_price)}</td>
                      <td className="px-4 py-3 text-right font-mono text-bear">{formatPrice(s.stop_loss)}</td>
...
function StatCard({
  ...
  return (
    <div className="rounded-xl border bg-card px-4 py-3 shadow-[var(--shadow-card)]">
      ...
      <div className={cn("font-display text-2xl font-bold mt-1 tabular-nums", accentClass)}>{value}</div>
    </div>
  );
}
